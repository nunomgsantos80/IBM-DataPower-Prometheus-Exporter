...
def get_status(dp, domain, endpoint):
    ...
    return call_curl(url, dp["user"], dp["password"])
...
def generate_metrics(config):
    ...
    for dp in config["appliances"]:
        ...
        # ====================================================
        #  MÉTRICAS GLOBAIS (fora dos domínios)
        # ====================================================
        ...
        if sys_data and "SystemUsage" in sys_data:
            ...
            log.warning("SystemUsage não disponível para %s", name)
        ...
        for dom in domains.get("DomainStatus", []):
            ...
            # ====================================================
            #  UPTIME (apenas no domínio default)
            # ====================================================
            if domain == "default":
                dt = cached_fetch(
                    f"{name}_uptime", 60,
